								...
								<ul>
									...
									<li style="padding: 0">
										<div class="currency_select">
											<select name="language" class="cls_lang" aria-labelledby="language-selector-label" id="js-language-select">
    @foreach ($language as $value => $label)
        <option value="{{ $value }}" {{ (Session::get('language') ? Session::get('language') : $default_language[0]->value) == $value ? 'selected' : '' }}>
            {{ $label }}
        </option>
    @endforeach
</select>
				...
				<div class="row text-center" style="margin-top: 40px;color: white;" style="color:white">
					<div class="pull-right hidden-xs">
				    </div>
				    <strong>Copyright &copy; {{$copyright_year}} <a href="{{$copyright_url}}" style="color: white;"> {{$copyright_name}} </a>.</strong> All rights
				    reserved.
			    </div>
			</div>
		</div>
