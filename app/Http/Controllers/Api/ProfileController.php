...
class ProfileController extends Controller
{
	...
	public function get_rider_profile(Request $request)
	{
       ...
       //Get Wallet Account number
        $userWallet = Wallet::where('user_id',$user_details->id)->first();
        if($userWallet){
            $walletAccount =$userWallet->accountno;
        }else{
             $walletAccount = "";
        }
		$user_data['wallet_amount'] = $wallet_amount;
		$user_data['wallet_account'] = $walletAccount;
	...
	public function get_driver_profile(Request $request)
	{
       ...
       //Get Wallet Account number
		...
		return response()->json([
			...
			'wallet_account' 	=> $walletAccount?? $user->id.'XXX',
