...
use App\Models\ManualRideRequest;
...
\Log::error('trip responce'. json_encode($return_data));
	 ...
	 */
      ...
      public function driver_create_ride(Request $request)
	{
		$user_details = JWTAuth::parseToken()->authenticate();
		$user = User::where('id', $user_details->id)->first();
		$user_location = DriverLocation::where('user_id', $user_details->id)->first();
		$apiKey = "AIzaSyC3KDJw18brcOSxfMIeN7OeZS2Sdc8SdKQ";
		...
		if($request->pickup_address==null){
                ...
                if ($data['status'] === 'OK') {
                    ...
                    // Handle the case when the geocoding request fails
                }
        ...
        // Get Latitude and Longitude for Pickup Address
        $dropOffresponse = $client->get("https://maps.googleapis.com/maps/api/geocode/json", [
            'query' => [
        ...
        if ($DropOffdata['status'] === 'OK' && !empty($DropOffdata['results'])) {
            $dropOfflatitude = $DropOffdata['results'][0]['geometry']['location']['lat'];
            $dropOfflongitude = $DropOffdata['results'][0]['geometry']['location']['lng'];

        }
    
		// Create Trip
		$Requesttrip = new ManualRideRequest;
		$Requesttrip->driver_id 		= $user_details->id;
		//$trip->pool_id 		= $pool_trip_id ?? '0';
		$Requesttrip->pickup_address 	= $pickupaddress;
		$Requesttrip->car_type 	=$user->car_type;
		$Requesttrip->dropoff_address	 = $request->destination_address;
		$Requesttrip->pickup_lat	=$latitude;
		$Requesttrip->pickup_lng	=$longitude;
		$Requesttrip->dropoff_lat	=$dropOfflatitude;
		$Requesttrip->dropoff_lng	=$dropOfflongitude;
		$Requesttrip->status	=1;
		$Requesttrip->save();
        ...
        $rider = User::where('mobile_number',$user->mobile_number)->where('user_type','Rider')->first();

        if ($rider==null) {
            $rider = new User;
            $rider->first_name  = $user->first_name;
            $rider->last_name   = $user->last_name;
            $rider->email       = $user->email;
            $rider->country_code= $user->country_code;
            $rider->country_id  = $user->country_id;
            $rider->mobile_number = $user->mobile_number;
	   ...
	   $fare_estimation = $this->request_helper->GetDrivingDistance($latitude, $dropOfflatitude, $longitude, $dropOfflongitude);
	   $polyline = @$this->request_helper->GetPolyline($latitude, $dropOfflatitude, $longitude, $dropOfflongitude);
	   $timezone = $this->request_helper->getTimeZone($latitude, $request->pickup_longitude);
	   
	   //Get Location Id
	   // Find location from pickup latitude & longitude
		$match_location = Location::select(
			DB::raw("id,status,(ST_WITHIN( ST_GeomFromText('POINT(".$latitude.' '.$longitude.")'),ST_GeomFromText(coordinates))) as available ")
	   ...
	   $CarType=CarType::where('car_name',$user->car_type)->first();
	 
	   //Get latest group_id
	   $latestRequest = RideRequest::latest()->first();
	   $group_id = $latestRequest->group_id;
	   
		//Insert record in RideRequest table
		
       
		$request = new RideRequest;
        $request->user_id = $rider->id;
        $request->group_id = $group_id + 1;
        $request->seats = 0;
        $request->pickup_latitude = $latitude;
        $request->pickup_longitude = $longitude;
        $request->drop_latitude = $dropOfflatitude;
        $request->drop_longitude = $dropOfflongitude;
        $request->driver_id = $user_details->id;
        $request->car_id = $CarType->id;
        $request->pickup_location = $pickupaddress;
        $request->drop_location = $Requesttrip->dropoff_address ?? ''; // Assign empty string if null
        $request->payment_mode = "Cash";
        $request->status = "Accepted";
        $request->timezone = $timezone;
        $request->schedule_id = '';
        $request->location_id = $location_id;
        $request->additional_fare = '';
        $request->peak_fare = 0;
        $request->additional_rider = 0 ?? fees('additional_rider_fare');
        $request->trip_path = @$polyline ?? '';
		...
		$trip->pool_id 			= 0;
		$trip->pickup_latitude 	= $latitude;
		$trip->pickup_longitude = $longitude;
		$trip->drop_latitude 	= $dropOfflatitude;
		$trip->drop_longitude 	= $dropOfflongitude;
		$trip->driver_id 		= $user_details->id;
		$trip->car_id 			= $CarType->id;
		$trip->pickup_location 	= $pickupaddress;
		$trip->drop_location 	= $Requesttrip->dropoff_address ?? '';
		$trip->request_id 		= $request->id;
		$trip->trip_path 		= @$polyline;
		$trip->payment_mode 	= "Cash";
		$trip->status 			= 'Scheduled';
		$trip->currency_code 	= "USD";
		$trip->peak_fare 		= 0;
		$trip->fare_estimation 	= $fare_estimation;
		$trip->additional_rider =  0;
		$trip->otp 				= mt_rand(1000, 9999);
		$trip->seats 			= 0;
		$trip->save();
		return response()->json([
			'status_code'		=>	'1',
			'status_message'	=>	__('Trip successfully Created'),
